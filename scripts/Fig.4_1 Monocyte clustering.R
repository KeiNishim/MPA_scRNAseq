...
#Road "CD14MonoCD16MonocDC1cDC2.rds" as s.int
Idents(s.int) <- "sample"
...
s.int <- subset(s.int, ident=c("MPA-1-pre", "MPA-1-post", "MPA-3-pre", "MPA-3-post","MPA-5-pre", "MPA-5-post")) 
s.int$sample <- factor(s.int$sample,
                       levels= c("MPA-1-pre", "MPA-1-post", "MPA-5-pre", "MPA-5-post"))
Idents(s.int) <- "celltypename"
DimPlot(s.int, group.by="celltypename",split.by="sample", shuffle=T, pt.size=0.1)

## add clustering step
Idents(s.int) <- s.int[["predicted.celltype.l2"]]
s.int <- subset(s.int, ident=c("CD14 Mono",hCD16 Monoh,"cDC1", "cDC2"))
...
s.int <- FindClusters(s.int, resolution=2.5, reduction="umap")
...
pp
ggsave('Fig.3F.png', pp, width=7.7, height=3.8)
write.csv(table(s.int@meta.data[["sample"]], s.int@meta.data[["seurat_clusters"]]), "monocyteprepost_clusters.csv")
